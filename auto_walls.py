...
class State:
    ...
    def _reset_state(self, user_wallpapers_dir: str, do_notify=False):
        ...
        if not os.path.exists(user_wallpapers_dir):
            raise FileNotFoundError(f"No such a directory: {
                                    user_wallpapers_dir} !")
        ...
        for w in os.listdir(user_wallpapers_dir):
            # complete dir for each wallpaper
            w = os.path.join(user_wallpapers_dir, w)
            if os.path.isfile(w):
                wallpapers.append(w)

        if not len(wallpapers):
            raise ValueError(f"There are no wallpapers in {
                             user_wallpapers_dir} !")
...
class AutoWalls(State):
    ...
    def set_wallpaper(self, wallpaper: str, do_change_index=True):
        ...
        try:
            ...
            cli = wallpapers_command.replace("<picture>", wallpaper).replace(
                "<display>", "").replace("<fit_mode>", "cover").split()

#            for i, cli_part in enumerate(cli):
#                cli_part = cli_part.replace(",", "").replace("'", "")
#                if cli_part == "<picture>":
#                    cli[i] = cli[i].replace("<picture>", wallpaper)
#                elif cli_part == "<display>":
#                    cli[i] = cli[i].replace("<display>", "")
#                elif cli_part == "<fit_mode>":
#                    cli[i] = cli[i].replace("<fit_mode>", "cover")
            ...
            themesw_color_scheme = os.path.expanduser(
                "~/.config/themesw/current")
...
#                )
    ...
    def is_timer_running(self):
        if not self.timer_pid or self.timer_pid == -1 or type(self.timer_pid) is int:
            return False
...
def get_wallpaper_thumbnail(
    ...
    if os.path.exists(wallpaper_thumbnail):
        ...
        subprocess.run([
            "ffmpeg",
            "-i", wallpaper_file,
            "-vf", f"scale='if(gt(iw,ih),{max_size},-1)':'if(gt(iw,ih),-1,{
                max_size})'",
            "-frames:v", "1",
            "-q:v", str(quality),
            "-update", "1",
...
def daemon(aw: AutoWalls):
    try:
        ...
        process = subprocess.Popen(
            [os.path.join(aw.script_dir, 'timer'), str(aw.config["interval"])])
...
def rofi(aw: AutoWalls, gen_thumbnails: bool):
    ...
    for wallpaper_file in aw.wallpapers:
        wallpaper_name = wallpaper_file.split("/")[-1]
        wallpaper_thumbnail = get_wallpaper_thumbnail(
            wallpaper_file, wallpaper_name)
    ...
    rofi_process = subprocess.Popen(
        ["rofi", "-dmenu", "-i", "-selected-row",
            str(aw.index), "-theme", rofi_theme],
    ...
    if selected_option:
        wallpaper = selected_option.split('\x00icon\x1f')[
            0]  # Extract wallpaper path
...
if __name__ == '__main__':
    ...
    rofi_p.add_argument("--gen-thumbnails",  action="store_true",
                        help="Generate thumbnails only")

    set_p = subps.add_parser("set",  help="Set given wallpaper")
    set_p.add_argument("wallpaper",  metavar="WALLPAPER",
                       type=str, help="Wallpaper path")

    next_p = subps.add_parser("next",   help="Set next wallpaper")
    prev_p = subps.add_parser("prev",   help="Set previous wallpaper")
    toggle_p = subps.add_parser(
        "toggle", help="Toggle background daemon on/off")
